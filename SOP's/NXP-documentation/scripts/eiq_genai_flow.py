...
#
# Added:
# - Native terminal performance output
...
class eIQGenAIFlow:
    ...
    def handle_question(self, question):
        ...
        if question != "":
            if self.query_classifier:
                ...
                print(f"[PERF - RAG] Knowledge Retrieval Time: {rag_time:.3f}s", flush=True)
                ...
                match query_category:
                    ...
                    case "INTENT":
                        ...
                        print(f"[PERF - RAG] Total Response Time: {total_time:.3f}s", flush=True)
                    case "REJECTED":
                        self.send_domain_response(random.choice(self.config.out_of_domain_response_list))
                    case "AMBIGUOUS":
    ...
    def get_llm_output(self, question, prompt):
        print("[DEBUG] ENTERED get_llm_output()", flush=True)
